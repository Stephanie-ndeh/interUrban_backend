...
class app extends Controller
{
        ...
        foreach ($this->dbStructure as $tableName => $columns) {
            ...
            foreach ($columns as $column => $properties) {
                if (!isset($properties["id"])) {
                    ...
                    $code["controller"] .= "public function delete(){\$this->deleteItems('base_model');}";
                }
                //
    ...
    private function controllerCode($tableName)
    {
        ...
        $cCode .= "\$this->model('" . $tableName . "_model',true);\$this->base_model->setDb(\$this->getDb());\$this->validation('v" . ucfirst($tableName) . "');}";

        $cCode .= $fPrefix . "get(){\$this->base_model->hydrater(\$_GET);\$this->responseJson(\$this->base_model->get());}";
        $addUpBody = "{if(\$this->v" . ucfirst($tableName) . "->run()){\$this->base_model->hydrater(\$_POST);if(\$this->base_model->add()){ \$this->responseJson(\$_POST);}else{\$this->responseJson();}}else{\$this->responseJson();}}";
        $cCode .= $this->genUpAddBody("add", $tableName);
        $cCode .= $this->genUpAddBody("update", $tableName);

        return $cCode;
    }
    private function genUpAddBody($type, $tableName)
    {
        return "public function " . $type . "(\$boolReturn = false){return \$this->gen" . ucfirst($type) . "('base_model', 'v" . ucfirst($tableName) . "', null, \$boolReturn);}";
    }
    private function declarate(string $name, string $type)
    {
        ...
        switch ($type) {
            ...
            case 'validation':
                ...
                break;
            case 'controller':
                $basicDeclaration .= "namespace app\controllers\api;use app\controllers\api\_primaries\primaryApi;class " . $name . " extends primaryApi{protected $" . $name . "_model;protected $" . "v" . ucfirst($name) . "; public function __construct(){";
                break;
            case 'schema':
